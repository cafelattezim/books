...
### 설계 추론(Speculate about Design)
#### 문제
소스 코드에서 설계 개념이 어떻게 표현되는지를 파악하는 것은 어렵다.

설계 개념이 다양하고, 프로그래밍 언어에서 표현하는 방법이 매우 많다.
소스 코드의 많은 부분이 설계와 무관한 구현(예: UI 제어, DB 연결 등)과 관련되어 있다.
하지만 이 문제를 해결하는 것은 가능하다.

시스템 기능을 대략적으로 이해하고 있다.
개발 경험이 있어 스스로 설계를 예상할 수 있다.
코드 구조에 대해 어느 정도 알고 있다.

#### 해결책
개발자의 전문성을 활용하여 가설적인 클래스 다이어그램을 만들고, 소스 코드와 비교하며 점진적으로 개선한다.

#### 절차
1. 초기 클래스 다이어그램 구성

   * 시스템을 이해한 바탕으로 예상되는 클래스, 속성, 메서드 등을 포함한 가설적인 클래스 다이어그램을 작성한다.
   * 네이밍 컨벤션 등을 고려하여 적절한 이름을 추측한다.
2. 소스 코드에서 클래스 다이어그램 요소 찾기

   * 클래스, 속성, 메서드 등의 이름을 나열하고 소스 코드에서 검색한다.
   * 이름이 일치하지 않을 수 있으므로 우선순위를 정해 검색한다.
3. 검증 및 수정

   * 코드에서 발견된 요소는 가설을 뒷받침한다.
   * 코드에서 찾을 수 없는 요소는 가설을 수정해야 한다.
4. 클래스 다이어그램 조정

   * 이름 변경: 소스 코드의 실제 명명과 다를 경우 이름을 수정한다.
   * 모델 재구성: 속성이 메서드로 표현되었거나, 메서드가 클래스로 표현된 경우 구조를 변경한다.
   * 확장: 다이어그램에 포함되지 않은 중요한 요소를 발견하면 추가한다.
   * 대체안 탐색: 코드에서 특정 개념을 찾을 수 없다면, 동의어 검색 또는 완전히 새로운 모델을 생성한다.
5. 반복 수행
   * 위 과정을 반복하여 만족스러운 클래스 다이어그램을 얻을 때까지 진행한다.

#### 장점
   * 확장성이 좋다.
     * 소스 코드에서 무엇을 찾을 것인지 추론하는 기법은 확장성이 뛰어나다.
     * 특히 100개 이상의 클래스를 가진 대형 객체 지향 프로그램에서는 바텀업 방식이 비효율적이므로, 이 기법이 유용하다.
   * 적은 투자로 큰 효과를 얻을 수 있다.
     * 이 기법은 도구나 자원 면에서 비용이 적게 들며, 이에 비해 높은 수준의 이해도를 제공한다.

#### 단점
* 전문성이 필요하다.
  * 이 기법을 효과적으로 활용하려면 다양한 관용구(idiom), 패턴, 알고리즘, 기술 등에 대한 폭넓은 지식이 필요하다.
  * 따라서 경험이 많은 전문가가 적용하는 것이 바람직하다.

* 시간이 많이 소요된다.
  * 자원과 도구 면에서는 효율적이지만, 만족할 만한 설계 모델을 도출하기까지는 상당한 시간이 필요하다.

#### 어려움(Difficulties)
* 일관성을 유지해야 한다.
  * 역공학(reverse engineering) 프로젝트가 진행됨에 따라 소프트웨어 시스템에 대한 이해가 깊어지므로, 클래스 다이어그램을 지속적으로 업데이트해야 한다.
  * 그렇지 않으면 초기 노력이 헛수고가 될 수 있다.
  * 이를 위해 소스 코드의 네이밍 컨벤션을 반영하여 다이어그램을 작성하고, 형상 관리 시스템(configuration management system)에 포함하여 관리하는 것이 중요하다.
